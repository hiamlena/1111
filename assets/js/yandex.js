...
const FALLBACK_FAVICON_DATA_URI =
  'data:image/svg+xml;base64,';
let faviconFallbackApplied = false;

function applyFaviconFallback(reason = 'timeout') {
  if (typeof document === 'undefined') return false;
  const head = document.head || document.querySelector('head');
  if (!head) return false;
  let link = document.querySelector('link#tt-favicon');
  if (!link) {
  ...
  if (currentHref === FALLBACK_FAVICON_DATA_URI) return false;
  link.setAttribute('rel', 'icon');
  link.setAttribute('type', 'image/svg+xml');
  link.setAttribute('href', FALLBACK_FAVICON_DATA_URI);
  link.removeAttribute('sizes');
  if (!faviconFallbackApplied) {
    console.info(
      `[TT] Favicon fallback активирован (${reason}) — используется встроенный SVG без сетевого запроса.`
    );
  }
  faviconFallbackApplied = true;
  return true;
}

/* ---------- Мониторинг favicon ---------- */
function monitorFaviconHealth() {
  if (typeof window === 'undefined' || typeof document === 'undefined') return;
  const queryIconLinks = () =>
    Array.from(
      document.querySelectorAll('link[rel~="icon"], link[rel="mask-icon"], link[rel="apple-touch-icon"]')
    );
  let iconLinks = queryIconLinks();
  const missingInitialLinks = iconLinks.length === 0;

  let resolved = false;
  let observer = null;
  const markLoaded = (href, entry = null) => {
    if (resolved) return;
    resolved = true;
    if (observer) {
      try { observer.disconnect(); } catch (e) {
        console.debug('[TT] Favicon observer disconnect error', e);
      }
    }
    const infoParts = [];
    if (entry && typeof entry.transferSize === 'number') {
      const fromCache = entry.transferSize === 0 && entry.encodedBodySize > 0;
      infoParts.push(fromCache ? 'из кеша' : `transfer=${entry.transferSize}`);
    }
    console.info(`%c[TT] Favicon обслуживается: ${href}`, 'color:#154baa;font-weight:600;', infoParts.join(', '));
    try {
      window.dispatchEvent(new CustomEvent('faviconLoaded', { detail: { href, entry } }));
    } catch (e) {
      console.debug('[TT] Не удалось отправить событие faviconLoaded', e);
    }
  };

  const fallback = reason => {
    if (resolved) return;
    const applied = applyFaviconFallback(reason);
    if (!applied) return;
    iconLinks = queryIconLinks();
    const fallbackLink = iconLinks.find(link => link.id === 'tt-favicon');
    const fallbackHref = fallbackLink ? fallbackLink.href : FALLBACK_FAVICON_DATA_URI;
    markLoaded(fallbackHref, { fallback: true, reason });
  };

  if (missingInitialLinks) {
    console.warn('[TT] Favicon: ссылки не найдены в <head>. Проверьте конфигурацию.');
    fallback('head-missing');
    return;
  }
  ...
  const hrefs = iconLinks
    ...
    .filter(Boolean);
  if (!hrefs.length) {
    console.warn('[TT] Favicon: не удалось определить адреса ресурсов.');
    fallback('invalid-href');
    return;
  }
    ...
    try {
      observer = new PerformanceObserver(list => {
        list.getEntries().forEach(entry => {
          const href = normalizedName(entry.name);
          if (hrefs.includes(href)) {
            markLoaded(href, entry);
          }
        });
      });
      ...
      console.debug('[TT] Favicon PerformanceObserver fallback', e);
    }
  }

  if (hrefs.some(href => href.startsWith('data:'))) {
    markLoaded(hrefs.find(href => href.startsWith('data:')));
    return;
  }
  ...
  window.addEventListener(
    'pageshow',
    () => {
      if (resolved || !window.performance?.getEntriesByType) return;
  ...
  window.setTimeout(() => {
    if (!resolved) {
      console.warn(
        '[TT] Favicon: не получено подтверждение загрузки. Убедитесь, что /favicon.ico отвечает 200 и кэш настроен корректно.'
      );
      fallback('timeout');
    }
  }, 4000);
